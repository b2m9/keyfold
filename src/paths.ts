...
export function compileOptions(options: MergeOptions): CompiledOptions {
  if (options.wireDeletes !== undefined && typeof options.wireDeletes !== "boolean") {
    throw new TypeError("keyfold: wireDeletes must be a boolean");
  }
  ...
  for (const policy of keyPolicies) {
    ...
    if (policy.segments.at(-1) === ITEM) {
      throw new TypeError(
        `keyfold: keyBy path '${policy.path}' addresses items, not a list; remove the trailing '[]'`,
      );
    }
  ...
  for (const policy of replacePolicies) {
    if (seenReplacePaths.has(policy.path)) {
      throw new TypeError(`keyfold: duplicate replace path '${policy.path}'`);
    }
    ...
    for (const keyed of keyPolicies) {
      if (isPrefix(policy.segments, keyed.segments)) {
        throw new TypeError(
          `keyfold: replace path '${policy.path}' makes keyBy path '${keyed.path}' unreachable`,
        );
      }
    }
...
function parsePath(path: string, source: "keyBy" | "replace"): string[] {
  ...
  for (const part of path.split(".")) {
    ...
    if (match === null) {
      throw new TypeError(
        `keyfold: invalid ${source} path '${path}'; use dot-separated properties with optional '[]' suffixes`,
      );
    }

    const property = match[1]!;
    if (UNSAFE_KEYS.has(property)) {
      throw new TypeError(`keyfold: path '${path}' contains reserved property '${property}'`);
    }
...
function validateIdentityField(path: string, field: string): void {
  if (typeof field !== "string" || !PROPERTY.test(field) || RESERVED_IDENTITY_FIELDS.has(field)) {
    throw new TypeError(`keyfold: keyBy path '${path}' must name a non-reserved identity field`);
  }
}
  ...
  for (const segment of segments) {
    if (segment === ITEM) {
      if (node.keyField === undefined) {
        throw new TypeError(
          `keyfold: path '${path}' addresses items of '${prefix}', but that list has no keyBy policy`,
        );
      }
    } else if (node.keyField !== undefined) {
      throw new TypeError(
        `keyfold: path '${path}' steps through keyed list '${prefix}' without the '[]' suffix`,
      );
    }
